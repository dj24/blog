...
- `ty: "zz"`: shape discriminator.
- `r`: ridges per segment as an animatable number.
- `s`: peak-to-trough distance as an animatable number.
- `pt`: point type identifier as an animatable number.
- Other data: unknown exporter-specific fields may also be present.

## Animated Attributes

- Animatable attributes here: `r`, `s`, `pt`.
